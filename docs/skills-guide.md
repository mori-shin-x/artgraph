...
> **前提:** `artgraph-setup` Skill は `artgraph init` の配布物です。この経路が使えるのは配布済み Skills が既にリポジトリにコミット済みの場合 (チームの誰かが init 済み) に限られます。チーム最初の 1 人は「CLI 経由」節の手動インストールが必要です — docs/getting-started.md の「Committing distributed Skills」節を参照。

配布済み Skills がコミットされたプロジェクトで (artgraph CLI 自体は未インストールでも) 「artgraph をセットアップしてほしい」と依頼すると、`artgraph-setup` Skill が起動し、以下を 1 ターンで完結させます:
