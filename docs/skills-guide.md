...
- Skills install (`--agents=<list>` で選択したエージェントの canonical skills path `<agent_skills_path>/<name>/SKILL.md` へ配置。`<agent_skills_path>` は `.claude/skills/` / `.agents/skills/` / `.cursor/skills/` / `.github/skills/` / `.kiro/skills/` のいずれか — 詳細は README の [Tier 1 cross-agent distribution](../README.md#tier-1-cross-agent-distribution))
...
| `<agent_skills_path>/<name>/SKILL.md` — `.claude/skills/` / `.agents/skills/` / `.cursor/skills/` / `.github/skills/` / `.kiro/skills/` のいずれか (`--agents=<list>` で選択したエージェントに対応) | `artgraph init` (default) の配備先、per-project |
