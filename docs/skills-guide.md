...
<!-- 注意: 直下の blockquote 内の getting-started.md への参照を markdown リンクにしないこと。docs/ は specDirs のため inline リンクが doc graph の depends_on エッジになり、推移的到達で spec 014 dogfood (tests/plan-coverage-dogfood.test.ts) が fail する。詳細は issue #254。 -->
> **前提**: `artgraph-setup` Skill は `artgraph init` の配布物です。この経路が使えるのは配布済み Skills が既にリポジトリにコミット済みの場合 (チームの誰かが init 済み) に限られます。チーム最初の 1 人は「CLI 経由」節の手動インストールを実行し、配布済み Skills をコミットしてください — `docs/getting-started.md` の「Committing distributed Skills」節を参照。

配布済み Skills がコミットされたプロジェクトで (artgraph CLI 自体は未インストールでも) 「artgraph をセットアップしてほしい」と依頼すると、`artgraph-setup` Skill が起動し、確認を挟んで以下を実行します:

1. パッケージマネージャ自動検出 (npm / pnpm / Bun / Deno; シグナル無し時のデフォルトと Yarn フォールバックは pnpm)
2. devDependency として install
3. `artgraph init` (full agent-native setup。コミット済みの `.artgraph.json` が既にある場合はスキップ)
4. `artgraph check` でセルフチェック

artgraph が既にインストール済みの場合は、再インストールせず現在のセットアップ状態をレポートします。
...
npx artgraph init --agents=claude       # エージェントを選択
npx artgraph init --minimal             # bare config のみ

# pnpm
pnpm add -D artgraph
pnpm exec artgraph init --agents=claude

# Bun
bun add -d artgraph
bunx artgraph init --agents=claude

# Deno
deno add npm:artgraph
deno run -A npm:artgraph/cli init --agents=claude
```
...
| `Files:` syntax | `src/auth.ts` | `src/auth.ts:validateToken` / `src/auth.ts:Sample.methodA` (file 単位も混在 OK) |
| 想定ユーザー | 新規実装 / 大規模 refactor | 既存の関数・メソッド 1 個だけ修正する保守ケース |
...
spec 019 の「同居しているだけでは巻き込まない」原則は、クラスの内部にも延長されています。symbol mode では、インライン export されたクラス (`export class` / `export default class`) の各メソッドが `symbol:<path>#ClassName.methodName` として独立の symbol node になります (default export クラスは `default.methodName`)。メソッド直上 (JSDoc の上でも可) の `// @impl REQ-NNN` はそのメソッド symbol に帰属し、`Files:` / CLI とも既存の `path:symbol` 構文のまま指定できます:

```markdown
Files: src/auth.ts:Sample.methodA
```

```bash
artgraph impact src/auth.ts:Sample.methodA
```

「1 クラス・複数メソッド・各メソッドが別 REQ を実装」という OOP 標準形で、メソッド 1 個だけ修正する保守ケースの過剰検知を抑制します。メソッドごとに別 REQ を追跡するためにクラスを standalone function 群へ分割する必要はありません。

粒度ごとのセマンティクス:

- **メソッド起点 (`:Sample.methodA`) はファイル内精度のクエリ** — `impactReqs` は当該メソッドの `@impl` claim とそこからの依存到達のみで、同じクラスの兄弟メソッドの REQ も、クラス宣言直上の `@impl` (クラス契約全体の宣言) も含まれません。また、クラスを import している依存元 (consumer) ファイルは `affectedFiles` に含まれません — consumer を含む爆風が必要な場面はクラス unit (`:Sample`) / file unit / `--diff` (file 単位 seed) を使ってください。トップレベル関数起点が逆 imports で consumer に到達するのとの非対称は意図的な裁定です (spec 021)。
- **クラス unit (`:Sample`) は全メソッドを巻き込む広い単位** — class→method の `contains` 辺 (spec 019 と同じ順方向限定トラバース) の展開により、クラス直上と全メソッドの REQ を含みます。file unit が file 内全 symbol を巻き込むのと同じ「広い単位は広い爆風」の関係です。クラスを import する consumer 起点の impact も、この経路で全メソッドの REQ に到達します。なお、クラス unit の `originReqs` はクラス直上タグの claim のみで、メソッドの claim は継承しません (file unit が子 symbol の claim を継承しないのと同一原則)。
...
- `path:ClassName.methodName` 形式 — クラスメソッド単位までさらに絞り込み(symbol mode 必須、spec 021 — 上記「クラスメソッド粒度」参照)
