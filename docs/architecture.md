...
> [growth-strategy.md](./internal/growth-strategy.md) §5 を参照）。★数はいずれも調査時点の概数。
...
- 注釈由来エッジは `GraphEdge.provenances` に `"annotation"` を含む。issue #35 で語彙統一済（[specs/011-edge-provenance/](../specs/011-edge-provenance/)）。現在は `annotation` / `frontmatter` / `convention` / `code-tag` / `task-tag` / `inline-link` / `ts-import` / `structural` / `coverage`（spec 020 で追加）の全 9 値。同一 `(source, target, kind)` を複数経路から生成した際は dedup union（昇順 sort）で 1 本に集約され、lock の `dependsOn` も `{id, provenances}` 形式に構造化される。
...
| `artgraph check [--gate] [--diff] [--base <ref>]`                  | 検証: drift（コード・doc 両方）/ orphan / 未カバー-vs-Plan / 未テスト。`--diff --gate` は変更が**新規に導入した**問題のみで exit 2（pre-existing 債務は baseline 差分でゲート対象外、spec 017 / issue #174）。`--base <ref>`（spec 023、`--diff` 必須）は CI 用: `git merge-base <ref> HEAD` を基準に、コミット間差分 ∪ 作業ツリー差分でゲートする（shallow clone 等で merge-base 不能なら fail-closed の exit 1）。baseline 構築不能時は exit 1。環境故障（例: oxc ネイティブバインディングのロード失敗 `OxcLoadError`）も「判定不能」として exit 1（非ブロック）— baseline 構築不能と同じファミリー。stderr に原因と対処法が出力される |
...
共通フラグ: `--diff`, `--gate`, `--format json|text`(file/symbol の粒度は CLI フラグでなく `.artgraph.json` の `mode` で設定する — #135)。`--base <ref>` は `check` 専用（`impact --base` は follow-up、spec 023 D3）。
...
- Stop（＝検証ゲート） → diff 全体を `check --gate`。drift（コード・ドキュメント両方）/ orphan / 未カバー / 未テストがあれば exit 2 で作業継続させる（stderr 再注入）。`stop_hook_active` を見て無限ループ回避。
  - 注記 (spec 023): Stop hook は**作業ツリー diff のまま**（`check --gate --diff`、`--base` は配布テンプレートに加えない）。CI の PR ゲートでは作業ツリーが clean なため、`check --diff --base origin/<base> --gate` でコミット間差分をゲートする。
