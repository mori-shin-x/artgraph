...
>
> 履歴メモ: コードネームは当初 `spectrace`（spec + trace）。名称衝突（spectrace.io / spectraceai.com）を避けるため Issue [#15](https://github.com/ShintaroMorimoto/artgraph/issues/15) で `artgraph`（artifact graph）に確定。本書内に残る `spectrace` は SDD 統合の成果物識別子（`.specify/extensions/spectrace/` / `.kiro/steering/spectrace.md`）のみ。
