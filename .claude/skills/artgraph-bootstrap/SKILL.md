...
2. **Impl tag** — a `// @impl REQ-NNN` line on the implementing function/class/module, one per REQ per symbol, using the target language's comment syntax. Placing it on the line directly above the exported declaration (or above its JSDoc) is fine under both `mode: "file"` and `mode: "symbol"` — in symbol mode the tag binds to the following symbol, not the file.
