...
契機: PR #299 (issue #286) で、`impact()` の BFS 意味論変更が `check --diff --gate` の scope filter (`buildScope`) を経由して evidence-only REQ を silent に見逃す **Finding 2 (HIGH)** が実装後のメタレビューまで発見されなかった。関数名 grep (`impact\(`) では transitive consumer が見えず、**戻り値フィールドの消費側**を追う必要があった(下記チェック 2 / Worked example 参照)。
...
背景: PR #299 の Option 2B は `exercises` 逆方向を潰したが、同じユーザー可視症状が `verifies` / `imports` 経路の hub-node bidirectional traversal で残った (#303)。例:
...
- (a) **greedy consumption**: `--flag` の直後に別フラグが来た場合に何が起きるか(commander は次のフラグを値として飲み込み、gate を無言解除しうる — PR #304 の F1/F2)
- (b) **空文字値**: `--flag ""` の挙動
- (c) **兄弟オプション横展開**: 同コマンドの他の値必須オプション (`--ignore` / `--format` など) に同型欠陥がないか(#306 の契機)
- (d) **fail-open 禁止**: parse 失敗時に gate が緩む方向に倒れないこと。repo convention は parse 時の `argParser` / `.choices()` 拒否

### 9. エラー原因の stage 帰属表(弱)

複数 stage が同一 failure channel(例: `baselineStatus: "unavailable"`)に合流する設計を導入・変更する場合、contract に**「原因 stage × ユーザー向けメッセージ」の対応表**を書き、誤帰属 (PR #304 の F3) をレビュー可能にする。該当しない PR ではスキップしてよい。
...
## Worked example — PR #299 Finding 2 (retro-test)

issue #286「`impact` の BFS 逆方向 `exercises` トラバース除去」の Step 0-pre で本 skill を回した場合、チェック 2 の `impactReqs` / `affectedDocs` / `affectedFiles` grep が以下を浮上させる:

- `src/commands/check.ts` の `buildScope` — `r.impactReqs` / `r.affectedDocs` / `r.affectedFiles` を scope filter に集約 (`src/commands/check.ts:363-371`)
- `src/plan-coverage/index.ts` — `impactResult.impactReqs` を coverage 判定に集約 (`src/plan-coverage/index.ts:624-627`)

→ 報告項目: 「BFS の reach を狭めると `check --diff --gate` の scope から evidence-only REQ が脱落し gate が silent に見逃す。影響 CLI: check / plan-coverage。ランク: HIGH」— これが実際には実装後メタレビューまで発見されなかった Finding 2 に相当する。**現行 6+3 チェックで事前検出可能なことを 2026-07-15 に検証済み。**

## フィードバックループ (Step 9 retro との接続)

issue 対応ループの Step 9 振り返りで「事前 (Step 0-pre) に見つけられたはずの finding」が特定された場合、その検出条件を本チェックリストへ追加する PR を出す。チェック 7 (hub-node, PR #299 retro) とチェック 8/9 (CLI parse / stage 帰属, PR #304 retro) はこのループの産物。
