...
// checklist-item rule and the two-consecutive-blank-lines rule it keeps
// `Files:` extraction inside one task block (e.g. `### T013`).
const HEADING_RE = /^#+\s/;
// Checklist item (`- [ ] ...` / `* [x] ...`). In Spec Kit's standard flat
// tasks.md (no per-task headings) the next checklist line is the next task,
// so it terminates the Stage A scope — without this rule a `Files:` block
// followed directly by the next task line swallows that task's description
// as a bogus file-path candidate (issue #219). A checklist marker can never
// be a valid `Files:` entry, so cutting the scope here loses nothing.
const CHECKLIST_ITEM_RE = /^\s*[-*]\s+\[[ xX]\]/;
...
function runStageA(lines: string[], options: ExtractOptions): StageAExtraction {
  ...
  while (i < lines.length) {
    ...
    // the next markdown heading, the next checklist item (issue #219 —
    // flat-checklist task boundary), or the second of two consecutive
    ...
    for (let j = scopeStart; j < lines.length; j++) {
      const line = lines[j];
      if (HEADING_RE.test(line) || CHECKLIST_ITEM_RE.test(line)) {
        scopeEnd = j;
        break;
      }
      if (line.trim() === "") {
