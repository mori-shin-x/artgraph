...
// oxc-parser based TypeScript extraction layer (issue #159, phase 1).
//
// Drop-in replacement for the ts-morph backend in typescript.ts: same
// createTSParser / parseTSFilePaths surface, same ParsedTS output — including
// node/edge ARRAY ORDER and contentHash values. Behavioral parity is pinned
// by tests/typescript-differential.test.ts, which runs both backends over the
// repo itself, the test fixtures, and edge-case probes and asserts deep
// equality. ts-morph was only load-bearing for three things, each re-derived
// here from empirically-probed compiler behavior:
//
//   1. Relative import/export specifier resolution
//      (`getModuleSpecifierSourceFile`). Probing showed the outcome depends
//      only on the tsconfig's jsx / allowJs / resolveJsonModule — NOT on
//      moduleResolution or package.json "type" (node16 strictness is not
//      enforced on this API), and that the resolved target must itself be a
...
//      `export const` for variables). See extractOxcSymbols.
...
//      import/export shape and is used as a fallback, mirroring ts-morph's
//      recovery for edges. (Symbol NODES in such files are a known
//      divergence: ts-morph recovers partial declarations, oxc does not.)
//
// The `@impl`/`[REQ]`/`req:` tag layer is plain-text regex and is shared with
// the ts-morph backend (imported above), so the tag grammar cannot fork.

// Like ts-morph, oxc-parser is a CJS package; loading it lazily via
// createRequire keeps this module's import cheap for callers that never
// parse (warm parse-cache runs).
  ...
  return {
    parse: (): ParsedTS => {
      ...
      for (const filePath of enumerateFiles(rootDir, patterns)) {
        const parsed = parseOxcFile(filePath, rootDir, mode, matchers, ctx);
...
// Parse exactly the given files (parse-cache miss path). Same contract as the
// ts-morph parseTSFilePaths: import resolution consults the real file system,
// so targets outside `filePaths` still resolve the way a full scan resolves
// them. Returns a fragment per input path.
export function parseTSFilePaths(
  ...
  for (const filePath of filePaths) {
    out.set(filePath, parseOxcFile(filePath, rootDir, mode, matchers, ctx));
  }
  return out;
}

// Mirror the ts-morph createTSParser flow: one glob per pattern (the same
// fast-glob call ts-morph's RealFileSystemHost makes inside
// addSourceFilesAtPaths), deduped, then ordered the way
// project.getSourceFiles() iterates its DirectoryCache.
function enumerateFiles(rootDir: string, patterns: string[]): string[] {
  ...
  return orderLikeTsMorphProject(files);
}

// ts-morph's LocaleStringComparer (used for sibling directory and file order).
...
// Replicate project.getSourceFiles() iteration order: ts-morph's
// DirectoryCache holds the directories of added files (plus the intermediate
// directories that connect a directory to an already-cached ancestor), and
// getSourceFilesByDirectoryDepth walks them breadth-first by path depth —
// "orphan" root directories in insertion order, child directories and files
// sorted by base name (LocaleStringComparer). Files are added per pattern in
// glob order, which fixes the orphan insertion order.
function orderLikeTsMorphProject(files: string[]): string[] {
...
function parseOxcFile(
  ...
  if (parsed && mode === "symbol" && !isTest) {
    symbolRanges = extractOxcSymbols(parsed.program, content, relPath, nodes);
  }
  if (parsed) {
    extractOxcImports(parsed, content, relPath, filePath, rootDir, edges, mode, isTest, ctx);
  }
...
// ts-morph reads files through the TS compiler host, which strips a UTF-8
// BOM — getFullText() (the file-hash input) and all node spans are relative
...
// ---------------------------------------------------------------------------
// Symbol-mode export extraction (getExportedDeclarations parity)
// ---------------------------------------------------------------------------

// A top-level declaration a local name can resolve to. `start`/`end` delimit
// the text ts-morph's getText() returns for the declaration node:
...
function extractOxcSymbols(
  ...
  const lookup = (name: string): LocalDecl | undefined => {
    ...
    // declarations first, so ts-morph's `declarations[0]` is the first
    // function when one exists, else the first declaration in source order.
  ...
  for (const stmt of program.body) {
    if (stmt.type === "ExportNamedDeclaration") {
      ...
      if (decl) {
        ...
        // skipped (ts-morph filters them out as foreign declarations).
      ...
      if (decl.type === "ClassDeclaration") {
        ...
        // when foo is imported/undeclared, matching ts-morph).
        const target = lookup(decl.name);
        if (target) push("default", target.start, target.end);
      } else {
        // `export default <expr>;` — ts-morph yields the expression node.
...
// which node ts-morph reports as each name's declaration:
...
// RestElement. The member's own span is the BindingElement text ts-morph
// hashes ("a", "b: c", "d = 1", "...rest"); nested patterns recurse to their
// leaves instead.
...
// (`export { a as "b-c" }`); ts-morph reports the literal's VALUE as the
// symbol name.
...
interface ImportRef {
  ...
  // Original (pre-alias) names of named imports, as source text — matches
  // ts-morph's getNameNode().getText() (string-literal names keep quotes).
  named: string[];
}

function extractOxcImports(
  ...
  if (parsed.program.body.length > 0) {
    ...
    // carries the import/export shape — use it so edges survive the way they
    // do under ts-morph's error recovery.
  ...
  // All import edges first, then re-export edges — ts-morph's extractImports
  // runs two separate statement loops in that order.
...
// ---------------------------------------------------------------------------
// Relative module specifier resolution (getModuleSpecifierSourceFile parity)
// ---------------------------------------------------------------------------

// Differential probing against ts-morph across tsconfig variants showed the
// resolution outcome depends only on these three options. In particular
// moduleResolution (node16 vs node10 vs bundler) and package.json "type" do
// NOT change it — the checker resolves extensionless relative specifiers even
// under node16, and .tsx / .js / .json availability is gated purely by
// jsx / allowJs / resolveJsonModule.
...
function createResolverContext(rootDir: string): ResolverContext {
  // Mirror buildProjectOptions: only <rootDir>/tsconfig.json is consulted
  // (no upward walk); without it, compiler defaults apply (all three off).
...
// Extension probe order replicated from ts-morph behavior:
...
// ---------------------------------------------------------------------------
// Line numbers (getStartLineNumber / getEndLineNumber parity: 1-based,
// counted at the trivia-free span offsets)
// ---------------------------------------------------------------------------
