...
**配布済み Skills がコミット済みのリポジトリに参加する場合:** 手動インストールをスキップできます — `/artgraph-setup` と打つだけで、Skill がパッケージマネージャを検出し、artgraph をインストールし、`init` までを 1 ターンで完了させます。チーム最初の 1 人は上記の手動インストールを実行し、配布済み Skills をコミットしてください — [Committing distributed Skills](./docs/getting-started.md#committing-distributed-skills) を参照。
