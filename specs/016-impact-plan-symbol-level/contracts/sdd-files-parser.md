...
- `TaskBlock` は heading 形式 (`### T013 ...`) に加え、flat checklist 形式 (`- [ ] T013 ...`) の task も検出する (issue #220 — task 数カウントと `requireFilesSection` の Spec Kit 標準テンプレ対応)。ID を持たない heading / checklist item は従来通り omit。
...
10. taskBlocks: flat checklist task (`- [ ] T001 ...`) を heading task と同様に検出し、次の task marker / heading までの scope 内の `Files:` 有無を `hasFilesSection` に反映 (issue #220)。ID 無し checklist item は omit。
