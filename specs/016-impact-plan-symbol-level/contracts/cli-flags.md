...
> → spec 024 (`specs/024-impact-base-ref/`, issue #305) が `--base <ref>` で変更ファイル集合を merge-base 起点のコミット間差分との和集合に拡張。なお `line` 値の実装は `0` ではなく `1` (`src/commands/impact.ts` / `pathsToEntries`) — 現行意味論は spec 024 `contracts/cli-impact-base.md` §1.1/§1.2 を参照。
