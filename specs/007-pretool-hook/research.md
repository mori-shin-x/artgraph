...
Rationale:
- artgraph の目的は「影響範囲の情報提供」であり、ツール実行のブロックは行わない
- exit 0 で常に返すことで、artgraph のエラーが Claude Code のワークフローを阻害しない
...
- Claude Code はツール呼び出し時にプロジェクトルートからの相対パスを渡すことが多いが、
  絶対パスを渡す場合もある
- artgraph のグラフノードはプロジェクトルートからの相対パスで管理されているため、
  一貫した形式に変換する必要がある
- `process.cwd()` は artgraph コマンドが実行されるディレクトリであり、
  通常はプロジェクトルートと一致する

Alternatives considered:
- `.artgraph.json` からルートを推定: 設定ファイルが存在しない場合に対応できない
- git リポジトリのルートを使用: git 依存を増やす。artgraph は git なしでも動作すべき
...
- stderr に `artgraph: hook-pretool completed in Xms` と出力（デバッグ用）

Rationale:
- v1 では個別コンポーネントの計測は不要。全体の応答時間のみで判断する
- 5 秒を超過する場合は P3 のデーモン化（`artgraph daemon`）で対応する
