...
- [ ] T007 [P] `tests/hook-pretool.test.ts` に `formatAdditionalContext()` のテストを追加する:
  - `affectedReqs: ["FR-001"]`, `affectedDocs: ["doc:api-design"]` → `"artgraph impact: FR-001 (req), doc:api-design (doc)"` が返ること
  - `affectedReqs: ["FR-001", "SC-001"]`, `affectedDocs: []` → `"artgraph impact: FR-001 (req), SC-001 (req)"` が返ること
  - `affectedReqs: []`, `affectedDocs: []` → `"artgraph impact: (none)"` が返ること
...
- [ ] T010 `src/hook-pretool.ts` に以下の関数をエクスポートする:
  ...
  - `formatAdditionalContext(result: ImpactResult): string` — affectedReqs を `ID (req)` 形式、affectedDocs を `ID (doc)` 形式でカンマ区切りに結合。reqs を先、docs を後に配置。両方空なら `"artgraph impact: (none)"` を返す
...
Purpose: `artgraph hook-pretool` サブコマンドの E2E テストを先に書く。
...
- [ ] T013 `tests/cli.test.ts` に graceful degradation テストを追加する:
  - `.artgraph.json` が存在しないディレクトリ（例: `/tmp`）で実行した場合、exit 0 で additionalContext が空文字列であること
...
- [ ] T015 `src/cli.ts` に `hook-pretool` サブコマンドを追加する。commander の `.command("hook-pretool")` で登録し、以下のフローを action 内に実装する:
  ...
  3. `parseHookInput(stdinText)` で JSON パース。null の場合は stderr に `artgraph: failed to parse hook input` を出力し、空 additionalContext の hookSpecificOutput を stdout に出力して exit 0
  ...
  8. `resolveStartIds(graph, filePaths)` で開始ノード ID を解決。空なら `"artgraph impact: (none)"` の additionalContext で exit 0
  ...
  13. stderr に `artgraph: hook-pretool completed in Xms` を出力（経過時間を計算）
...
Checkpoint: `artgraph hook-pretool` サブコマンドが動作し、E2E テストが PASS する。
...
- `loadConfig()` は `.artgraph.json` がなくてもデフォルト設定を返すが、spec ディレクトリが実際に存在しない場合は空 additionalContext で早期終了する
- additionalContext は `artgraph impact: FR-001 (req), doc:api-design (doc)` 形式。file ノードは含めない
