# Contract: artgraph hook-pretool

## コマンド概要

`artgraph hook-pretool` は Claude Code の PreToolUse hook として機能するサブコマンド。
...
```json
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "artgraph impact: FR-001 (req), SC-001 (req), doc:api-design (doc)"
  }
}
```

additionalContext のフォーマット:
- `artgraph impact: ` プレフィックスの後に、影響を受けるノードをカンマ区切りで列挙
...
- .artgraph.json が存在しない
- stdin の JSON パースに失敗
- tool_input に file_path が含まれない
- scan やimpact でエラーが発生
- グラフに対象ファイルのノードが存在しない

理由: artgraph のエラーが Claude Code のワークフローをブロックしてはならない。
...
|--------|-------------------|-------------|
| .artgraph.json 不在 | `""` | なし |
| stdin 読み取り失敗 | `""` | `artgraph: failed to read stdin` |
| JSON パース失敗 | `""` | `artgraph: failed to parse hook input` |
| file_path 不在 | `""` | なし |
| scan 失敗 | `""` | `artgraph: scan failed: {error}` |
| impact 失敗 | `""` | `artgraph: impact failed: {error}` |
| 正常（影響あり） | `"artgraph impact: FR-001 (req), ..."` | `artgraph: hook-pretool completed in Xms` |
| 正常（影響なし） | `"artgraph impact: (none)"` | `artgraph: hook-pretool completed in Xms` |
