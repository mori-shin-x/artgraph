    ...
    steps:
      ...
      - name: Select and run tests (fall back to full suite on exit 1)
        run: |
          ...
          # PR がテストファイル自体を削除した場合、選択が存在しないパスのみになり
          # `vitest run <なし>` が exit 1 で正当な PR を落とす (PR #316 review AG-2) — 存在するものだけ渡す
...
- 削除された / グラフ未追跡の変更ファイル、および base..HEAD 内で rename されたファイル (旧パス基準の stale evidence とは join しない — PR #316 review AG-1) は選択に寄与しない (spec D-1 / FR-007 — 宣言された選択限界)。それらの正しさは同じ CI の `check --diff --base --gate` ステップが exit 2 で捕まえる。
...
### rename されたファイルのテストが選択されない

base..HEAD 内で rename されたファイルは new path で startId を解決するが、base ブランチでキャッシュした trace shards は evidence を **旧パス** で記録しているため、`--tests` の path 文字列 join が繋がらず、そのテストは非空・exit 0 の選択から無言で落ちる (PR #316 review AG-1 — FR-007 の宣言された選択限界)。正しさは `check --diff --base --gate` が担保する。不確かなら full suite に fallback する。回復手段は research.md R1 の trace-join follow-up 候補。
