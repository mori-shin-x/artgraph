...
- **FR-007**: startId 解決は **現在グラフのみ** で行う (D-1)。base..HEAD 内で削除されたファイルは merged diff に現れても startId を解決せず、エラー・警告なしで寄与しない。同様に、base..HEAD 内で **rename されたファイル** は new path で startId を解決するが、旧パス基準の stale evidence (base ブランチで生成・キャッシュした trace shards は旧パスで evidence を記録している) とは path 文字列で join しないため、そのテストは無言で選択から落ちる (PR #316 review AG-1) — これも宣言された選択限界に含める。回復手段は research.md R1 の trace-join follow-up (削除・rename の両方を回復する)。baseline worktree・baseline グラフ側の startId 解決を導入してはならない。これは宣言された選択限界であり、FR-009 の consumer rule とセットでドキュメント化する。
...
- **FR-009**: consumer rule をドキュメント (README / docs/commands.md / SKILL.md / output-schema.md) に明記しなければならない (D-5): 「削除された / グラフ未追跡の変更ファイル、および base..HEAD 内で rename されたファイル (旧パス基準の stale evidence とは join しない — PR #316 review AG-1) は選択に寄与しない。`impact --tests` は最適化として扱うこと — exit 1 時や不確かな場合は full suite に fallback する。正しさのゲートは `check --diff --base --gate` のまま。」
