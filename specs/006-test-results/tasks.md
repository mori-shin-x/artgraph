...
- [ ] T002 [P] `tests/fixtures/test-results/vitest-pass.json` を作成する。内容: `[REQ-001]` を含むテストが passed の Vitest JSON レポーター形式
- [ ] T003 [P] `tests/fixtures/test-results/vitest-fail.json` を作成する。内容: `[REQ-001]` を含むテストが failed の Vitest JSON レポーター形式
- [ ] T004 [P] `tests/fixtures/test-results/vitest-describe-inherit.json` を作成する。内容: describe ブロックに `[REQ-001]` があり、内部のテストケースに REQ タグがない形式
- [ ] T005 [P] `tests/fixtures/test-results/vitest-multi-req.json` を作成する。内容: テスト名に `[REQ-001][REQ-002]` の複数 REQ タグがある形式
- [ ] T006 [P] `tests/fixtures/test-results/vitest-skip.json` を作成する。内容: `[REQ-001]` を含むテストが skipped の Vitest JSON レポーター形式
- [ ] T007 [P] `tests/fixtures/test-results/vitest-namespaced.json` を作成する。内容: `[001-auth/FR-001]` の名前空間修飾 REQ タグを含む形式
- [ ] T008 [P] `tests/fixtures/test-results/junit-pass.xml` を作成する。内容: `[REQ-001]` を含む testcase が pass の JUnit XML
- [ ] T009 [P] `tests/fixtures/test-results/junit-fail.xml` を作成する。内容: `[REQ-001]` を含む testcase が failure の JUnit XML
- [ ] T010 [P] `tests/fixtures/test-results/junit-suite-inherit.xml` を作成する。内容: testsuite name に `[REQ-002]` があり、内部 testcase に REQ タグがない形式
- [ ] T011 [P] `tests/fixtures/test-results/invalid-format.txt` を作成する。内容: JSON でも XML でもない不正なテキスト
...
- [ ] T012 `tests/test-results.test.ts` を新規作成し、`extractReqTags()` のテストを記述する:
...
- [ ] T013 `tests/test-results.test.ts` に `parseVitestJson()` のテストを追加する:
  ...
  - 不正な JSON 文字列 → 空配列を返す
- [ ] T014 [P] `tests/test-results.test.ts` に `parseJUnitXml()` のテストを追加する:
  ...
  - 不正な XML 文字列 → 空配列を返す
- [ ] T015 [P] `tests/test-results.test.ts` に `parseTestResults()` のテストを追加する:
  - JSON コンテンツ → `parseVitestJson()` が呼ばれる（先頭 `{` で判別）
  - XML コンテンツ → `parseJUnitXml()` が呼ばれる（先頭 `<` で判別）
  - 不正コンテンツ → 空配列を返す
- [ ] T016 [P] `tests/test-results.test.ts` に `buildTestResultMap()` のテストを追加する:
...
- [ ] T017 `src/test-results.ts` を新規作成し、以下の関数をエクスポートする:
...
- [ ] T018 Phase 2 のテスト（T012-T016）が全て PASS することを確認する（`pnpm test tests/test-results.test.ts`）
...
- [ ] T019 `tests/coverage.test.ts` を新規作成し、以下のテストを記述する:
...
- [ ] T020 `src/coverage.ts` の `computeCoverage()` を拡張する:
...
- [ ] T021 T019 のテストが全て PASS することを確認する
...
- [ ] T022 `tests/test-results.test.ts` に `loadTestResults()` のテストを追加する:
...
- [ ] T023 `tests/test-results.test.ts` に複数ファイル統合のテストを追加する:
...
- [ ] T024 `src/test-results.ts` に `loadTestResults()` を追加する:
...
- [ ] T025 T022, T023 のテストが全て PASS することを確認する
...
- [ ] T026 `src/config.ts` の `loadConfig()` を変更し、`testResultPaths` フィールドを読み込む
- [ ] T027 `src/check.ts` の `check()` にオプショナルな `testResults?: TestResultMap` 引数を追加し、`computeCoverage(graph, testResults)` に渡す
- [ ] T028 `src/cli.ts` の `check` コマンドに `--test-results <paths...>` オプションを追加する:
...
- [ ] T029 [P] `src/cli.ts` の `coverage` コマンドに `--test-results <paths...>` オプションを追加する:
  - `computeCoverage(graph, testResults)` にテスト結果を渡す
  - テスト結果読み込みロジックは check と同一パターン
- [ ] T030 [P] `src/cli.ts` の `scan` コマンドに `--test-results <paths...>` オプションを追加する:
  - テスト結果の統計情報（テスト結果ファイル数、マッチした REQ 数、pass/fail 数）をスキャン出力に含める
- [ ] T031 `tests/cli.test.ts` に以下の CLI 統合テストを追加する:
...
- [ ] T032 全テスト PASS、`pnpm build` 成功を確認する
...
- [ ] T033 `tests/cli.test.ts` に設定ファイルベースのテストを追加する:
...
- [ ] T034 `src/test-results.ts` の `loadTestResults()` を拡張し、glob パターンをサポートする:
  - glob パターン（`*`, `**`）を展開してファイルパスリストに変換
  - 既存の `glob` パッケージ（依存済み）を使用
- [ ] T035 T033 のテストが全て PASS することを確認する
...
- [ ] T036 全テスト（`pnpm test`）がパスし、ビルド（`pnpm build`）が成功することを確認する
- [ ] T037 以下のシナリオを手動で検証する:
