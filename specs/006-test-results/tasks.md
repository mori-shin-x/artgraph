...
- [x] T001 `src/types.ts` に以下の型を追加する:
  ...
  - `ArtgraphConfig.testResultPaths?: string[]`
...
- [x] T031 `tests/cli.test.ts` に以下の CLI 統合テストを追加する:
  - `artgraph check --test-results vitest-pass.json` → exit 0、coverage に `verified` が含まれる
  - `artgraph check --test-results vitest-fail.json` → coverage に `impl-only` が含まれる
  - `artgraph check` (テスト結果なし) → 従来と同一の結果
  - `artgraph coverage --test-results vitest-pass.json` → `verified` 表示
  - `artgraph coverage --test-results vitest-fail.json` → `impl-only` 表示
  - `artgraph coverage` (テスト結果なし) → 従来と同一の結果
...
Purpose: `.artgraph.json` の `testResultPaths` フィールドによるテスト結果パス指定を実装する。

- [x] T033 `tests/cli.test.ts` に設定ファイルベースのテストを追加する:
  - `.artgraph.json` に `testResultPaths: ["fixtures/test-results/*.json"]` を設定 → テスト結果が自動的に読み込まれる
  - `.artgraph.json` の `testResultPaths` と `--test-results` オプションが両方指定 → CLI オプションが優先される
...
- [x] T037 以下のシナリオを手動で検証する:
  - `artgraph check --test-results` で Vitest JSON のテスト結果が正しく反映される
  - `artgraph check --test-results` で JUnit XML のテスト結果が正しく反映される
  - `artgraph coverage --test-results` で check と同一の判定結果が得られる
...
6. STOP and VALIDATE: `artgraph check --test-results` で手動検証
