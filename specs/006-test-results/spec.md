...
`spectrace check --test-results vitest-report.json` を実行すると、テスト名や describe ブロック名に含まれる `[REQ-xxxx]` タグが抽出され、対応する REQ ノードのテスト pass/fail 状態が判定に反映される。
...
Independent Test: `[REQ-001]` タグを含むテストが pass/fail する Vitest JSON レポートを用意し、`spectrace check --test-results` で `verified` / `impl-only` が正しく判定されることを確認する。

Acceptance Scenarios:

1. Given Vitest JSON レポートに `[REQ-001]` を含むテストが pass で記録されており、グラフ上に REQ-001 への `verifies` エッジがある, When `spectrace check --test-results vitest-report.json` を実行する, Then REQ-001 のカバレッジ状態が `verified` と報告される
2. Given Vitest JSON レポートに `[REQ-001]` を含むテストが fail で記録されており、グラフ上に REQ-001 への `verifies` エッジがある, When `spectrace check --test-results vitest-report.json` を実行する, Then REQ-001 のカバレッジ状態が `impl-only` と報告される
3. Given `verifies` エッジが存在するが Vitest JSON レポートに対応するテスト結果が存在しない, When `spectrace check --test-results vitest-report.json` を実行する, Then REQ-001 のカバレッジ状態が `impl-only` と報告される
4. Given テスト名の describe ブロックに `[REQ-001]` があり、その describe 内の個別テストケースに REQ タグがない, When `spectrace check --test-results vitest-report.json` を実行する, Then describe ブロックの REQ タグが内部のテストケースに継承され、全テスト pass なら `verified` となる
...
開発者が `vitest run --reporter=junit` で出力された JUnit XML、または他のテストランナーが出力した JUnit XML を持っている場合、`spectrace check --test-results junit-report.xml` を実行すると、Vitest JSON と同じロジックでテスト結果が取り込まれる。

Why this priority: JUnit XML は CI パイプラインの事実上の標準フォーマット。CI 上で `spectrace check --gate` を実行する際、テスト結果の取り込みが JUnit XML 経由でできないと実用にならない。Vitest JSON と同等の優先度。

Independent Test: `[REQ-001]` タグを含む JUnit XML レポートを用意し、`spectrace check --test-results` で正しく判定されることを確認する。

Acceptance Scenarios:

1. Given JUnit XML レポートに `[REQ-001]` を含むテストケースが pass で記録されており、グラフ上に `verifies` エッジがある, When `spectrace check --test-results junit-report.xml` を実行する, Then REQ-001 のカバレッジ状態が `verified` と報告される
2. Given JUnit XML レポートに `[REQ-001]` を含むテストケースが failure で記録されている, When `spectrace check --test-results junit-report.xml` を実行する, Then REQ-001 のカバレッジ状態が `impl-only` と報告される
3. Given JUnit XML の testsuite name に `[REQ-002]` があり、内部の testcase に個別の REQ タグがない, When `spectrace check --test-results junit-report.xml` を実行する, Then testsuite の REQ タグが内部の testcase に継承される

---

### User Story 3 - テスト結果なしでの後方互換動作 (Priority: P1)

既存ユーザーが `--test-results` オプションを指定せずに `spectrace check` を実行した場合、従来と全く同じ動作をする。`verifies` エッジの有無だけで `verified` を判定し、テスト pass/fail は考慮しない。

Why this priority: 後方互換性の保証は段階的導入（Incremental Adoption）の原則に基づき、既存ワークフローを壊さないために必須。

Independent Test: テスト結果オプションなしで `spectrace check` を実行し、現在の挙動と同一の結果が得られることを確認する。

Acceptance Scenarios:

1. Given `verifies` エッジが存在する REQ がある, When `spectrace check` を `--test-results` なしで実行する, Then REQ のカバレッジ状態は従来通り `verified` と報告される（テスト pass/fail は考慮されない）
2. Given `.spectrace.json` に `testResultPaths` が設定されていない, When `spectrace check` を実行する, Then テスト結果の取り込みは行われず、従来の判定ロジックで動作する
...
1. Given `.spectrace.json` に `"testResultPaths": ["test-results/*.json"]` が設定されている, When `spectrace check` をオプションなしで実行する, Then 指定パスのテスト結果ファイルが自動的に読み込まれ、判定に反映される
2. Given `.spectrace.json` に `testResultPaths` が設定されており、かつ `--test-results` オプションも指定されている, When `spectrace check` を実行する, Then CLI オプションの指定が設定ファイルを上書きする
3. Given `testResultPaths` で指定されたパスにファイルが存在しない, When `spectrace check` を実行する, Then テスト結果なしとして従来の判定で動作する（エラーにはならない）
...
1. Given 2つのテスト結果ファイルがあり、ファイル A に `[REQ-001]` の pass、ファイル B に `[REQ-002]` の pass が記録されている, When `spectrace check --test-results fileA.json --test-results fileB.json` を実行する, Then REQ-001 と REQ-002 の両方が `verified` と報告される
2. Given 2つのテスト結果ファイルに同じ `[REQ-001]` のテストがあり、一方が pass で他方が fail, When 両ファイルを指定して `spectrace check` を実行する, Then REQ-001 は `impl-only` と報告される（1つでも fail があれば verified にならない）
...
- FR-001: `spectrace check` コマンドは `--test-results <path>` オプションを受け付け、指定されたテスト結果ファイルを読み込んで coverage 判定に使用する
...
- FR-006: `SpectraceConfig` に `testResultPaths` フィールドを追加し、設定ファイルでテスト結果ファイルのパスを指定できるようにする
...
- testResultPaths: `SpectraceConfig` に追加される設定フィールド。テスト結果ファイルのパスまたは glob パターンの配列
...
- テスト名や describe ブロック名に `[REQ-xxxx]` 形式でタグを埋め込むのは開発者の責務であり、spectrace はタグの存在を前提に join を行う
