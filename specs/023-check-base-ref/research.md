...
**`-M` (rename 検出) を name-only に付ける理由**: rename を R レコードとして畳み込み、old/new 両 path が name-only 出力に現れる (git は rename の両側を出す) — startId 解決は new path、baseline 解決は inverse-rename 経由で old path を使うため、両方が集合に入っていて害はなく、取りこぼしがない。
