...
`artgraph rename` サブコマンドを新規に追加し、仕様 ID のリネーム・分割・統合を
...
| IV. CLI-First Interface | Pass | `artgraph rename` サブコマンドとして公開。`--dry-run`, `--format` オプション |
...
`src/cli.ts` に `artgraph rename --from <old> --to <new> [--dry-run] [--format json|text]` を追加。
