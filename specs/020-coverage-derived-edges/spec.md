...
- **FR-017**: `impact` は `exercises` エッジを到達経路として辿り、静的経路と証拠経路の由来を出力で区別できること。stale な exercises エッジは `staleness: exclude` 時は辿らないこと。

  **issue #361 (二層伝播 + 照合述語) で確定した現行トラバースポリシー** — #286 Option 2B / #303 の per-edge な「evidence-only なら許可」ヒューリスティックは、複数 origin が 1 つの evidence-only REQ に収束するケース (HIGH-2) や、evidence-only REQ 同士が同一ハブ経由で互いを漏らすケース (#322 known limitation) を構造的に防げなかった。#361 はこれを「宣言されたエッジ (strong) は推移的、偶然の観測エッジ (weak) は収集はするが再展開しない (collect-terminal)」という二層モデルに置き換えた:
  - `implements`: 双方向 strong(常時)。
  - `verifies`: 双方向 strong。ただし reverse `verifies`/`imports` で `kind: test` ノードへ到達した「制限付きテストハブ」自身の forward `verifies` は、target REQ が `reqsWithImplements` に含まれない (evidence-only) 場合に限り、かつ **照合述語**(後述)を満たす場合のみ weak(terminal)で許可する。target REQ が `implements` 辺を持つ場合は不許可(#303 の R3a を維持)。
  - `imports`: 双方向 strong。ただし制限付きテストハブ自身の forward `imports` は常時不許可(#303 の R3b を維持)。
  - `contains`(doc→req|task): forward のみ strong、reverse は常時不許可(issue #215、spec 019 FR-001〜003 を維持、変更なし)。
  - `depends_on` / `derives_from`: 辺の provenance に frontmatter/annotation 等の明示宣言を 1 つでも含めば strong(双方向・推移的)。provenance が inline-link のみの辺は weak(terminal)——著者が偶然書いたマークダウンリンクは宣言された依存関係ではない(issue #254 の無制限ファンアウトを構造的に閉じる)。
  - `exercises`(req→symbol|file): forward・reverse とも weak(terminal)に変更(forward は #300 のカスケード漏れ対策、reverse は #286 Option 2B の到達可否判定 — `reqsWithImplements` を持つ REQ からの reverse は不許可 — を維持しつつ terminal 化)。

  **照合述語 (matching predicate)**: 制限付きテストハブの forward `verifies` が収集する evidence-only REQ `R` は、`R` 自身の `exercises` エッジが、その `impact()` 呼び出しの BFS 起点集合(startIds + file→symbol 展開分。BFS 開始時に確定する不変集合であり、トラバース中に成長する `visited` 状態からは計算しない)内の file/symbol に到達している場合のみ収集する。単なるハブ共有だけでは収集条件を満たさない。

  **shard-less フォールバック (issue #363 H1)**: 照合述語は `exercises` エッジの存在に依存するため、trace shard を一度も ingest していないプロジェクト(グラフ全体に `exercises` エッジが 1 本も存在しない)では原理的に充足不能であり、無条件適用すると全ての evidence-only REQ が制限付きハブ経由で恒久的にブロックされる(#286 Option 2B の保証の消失、`check --diff --gate` では false-green の原因になる)。**グラフ全体に `exercises` エッジが 1 本も存在しない場合のみ**、照合述語を無効化し #303 時代の「ハブ共有のみで収集(terminal)」に fail-open する。グラフ内に `exercises` エッジが 1 本でも存在すれば照合述語は再びグラフ全体で必須になる — 部分的にしか trace を導入していないプロジェクトでは、証拠を持つ REQ と持たない REQ が同一ハブを共有する場合に後者が引き続き除外される残存限界がある(tracked as a follow-up issue)。

  **terminal 意味論**: weak に到達したノードは `visited` に "terminal" 状態で入り、`impactReqs`/`affectedFiles`/`affectedDocs`/`reqProvenance` および post-BFS attribution(doc の 1-hop 付与)の対象にはなるが、enqueue されず、そこから先へは展開されない。同一呼び出し内で後から strong 経路により到達した場合は "expandable" へ昇格し、フル再展開される(二重カウントなし — `visited` の KEYS のみが集計対象という不変条件は維持)。

  `maxDepth` の新しい既定値は導入しない(ノイズ源は weak 層で構造的に止まるため)。この節の外部 JSON 契約(`reqProvenance` の `"static"`/`"evidence"` 2 分類)は不変。full provenance-aware な条件付き traversal は #298 で追跡(#361 は #298 のスコープ全体を代替するものではなく、#286/#303/#322 が指摘した具体的な漏れの構造的閉鎖)。詳細は `src/graph/traverse.ts` のファイルヘッダコメント(「Current model (issue #361)」節)と `classifyEdgeTraversal` 関数を参照。
