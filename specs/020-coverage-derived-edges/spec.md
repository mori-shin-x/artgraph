...
- **FR-017**: `impact` は `exercises` エッジを到達経路として辿り、静的経路と証拠経路の由来を出力で区別できること。stale な exercises エッジは `staleness: exclude` 時は辿らないこと。traversal は REQ→symbol の**順方向は常に**辿り、symbol→REQ の**逆方向**は「source REQ が `implements` 辺を全く持たない (evidence-only, `acceptExercises: true` 運用) 場合のみ」許可する (#286 Option 2B)。`implements` 辺を持つ REQ に対する逆方向は sibling-REQ leak (incidentally-called symbol → 無関係 REQ) を防ぐため skip する。full provenance-aware な条件付き traversal は #298 で追跡。**issue #303 追記**: Option 2B は `exercises` の逆方向のみを塞いだが、`verifies` / `imports` は依然無条件双方向のため、テストファイル(1 ファイルが複数 sibling REQ を `verifies` し複数 sibling シンボルを `imports` する)がハブノードとなり同じ leak が別経路で再現する。対策として、reverse `verifies`/`imports` で `kind: test` ノードへ到達した場合をその到達に限り「制限付きテストハブ」とし、当該ハブ自身の forward `verifies` は target REQ が `reqsWithImplements` に含まれない(evidence-only)場合のみ許可し、forward `imports` は常に不許可とする。REQ 自身や test ノードを startId とした到達、および test 以外のノードへの reverse `verifies`/`imports` は無制限のまま。詳細は `src/graph/traverse.ts` のファイルヘッダコメント(issue #303 節)を参照。
