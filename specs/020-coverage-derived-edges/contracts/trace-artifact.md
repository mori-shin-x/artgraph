...
- **注(capture engine v2 / spec 021)**: 採取エンジンが計装方式(`engine: 'instrument'`、既定)の場合、`fn` は「V8 functionName 互換の命名規則で静的に決定した関数名」(実行時の V8 報告値ではなく AST からの静的決定)。`schemaVersion` は 1 のまま据え置き
