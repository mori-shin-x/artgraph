...
- **メソッドシンボル ID と既存シンボル名の衝突**: `export { helper as "Sample.methodA" }` (文字列エクスポート名) は現行 main でもドット入りシンボル ID を生成するため、クラスメンバーと同一 ID が衝突しうる。衝突時は**クラスメンバー側を優先**し、破棄した側について build warning を emit する (無言の先勝ちにしない)。warning は構造化された `BuildWarning` (`class-member-collision`) としてパーサ fragment に載って builder へ流れる — parse-cache のウォームヒットでも毎ビルド再報告され、`--format json` の `warnings[]` にも現れる (PR #242 レビュー A)。**敗者側の宣言に付いていた `@impl` タグは file 粒度へ降格せず、勝者のクラスメンバーシンボルへ再帰属する** — 敗者の attribution range は温存され (シンボルノードだけ抑止)、その range が解決する名前がそのまま勝者の ID になるため (PR #242 レビュー B)。
- **クラス名自体の seen 衝突 (同名クラス 2 個 / `export { x as "Sample" }` とクラス名の衝突)**: export 名の重複登録は pre-existing の一般制約 (登録順先勝ちで後発 entry を無言破棄) をそのまま継承する — 挙動は変えない。ただしクラス entry が絡む場合 (握り潰される側または勝者側が classMembers を持ち、かつ宣言スパンが異なる場合) はメンバーシンボル一式が丸ごと消えうるため、同じ `class-member-collision` warning で可視化する (PR #242 レビュー C)。`export function f(){}` + `export { f }` のような同一宣言スパンへの benign な重複 push は警告しない。
- **同一行に複数メンバー + トレーリングタグ**: `methodA(): void {} methodB(): void {} // @impl REQ-N` のように 1 行に複数メンバーが開く場合、各メンバーの attribution range が同サイズで tie になり、pre-existing の「登録順で先勝ち」により**最初のメンバー**に帰属する (新規機構ではなく既存 tie-break の継承)。oxfmt / prettier 等のフォーマッタで 1 行 1 メンバーに整形すれば自然に解消される。
- **文字列/数値リテラルのメンバー名 (`"foo"() {}` / `123() {}`)**: key が `Identifier` でないため computed property name と同様に**シンボル化せず**、タグはクラスシンボルへ帰属する (FR-004)。
- **`declare class`**: `export declare class` は ambient 宣言でありメンバーに実行時の実体がないため、**メンバー抽出を行わない** (クラスシンボルのみ、メンバー直上のタグはクラス帰属 — FR-001/FR-004、PR #242 レビュー D1)。
...
- **FR-004**: 次のメンバーはシンボル化せず、タグはクラスシンボルへ帰属する: computed property name、**文字列/数値リテラルのメンバー名 (`"foo"() {}` / `123() {}`)**、private `#member`、データプロパティ (関数値でない initializer)、`accessor`、abstract / `declare` メンバー、static block。**`declare class` は全メンバーが対象外** (クラスノード自身の `declare` フラグでガード — PR #242 レビュー D1)。非 export クラス・分離/alias export クラス・クラス式・ネストクラスは対象外 (挙動不変)。
