...
新規プロジェクトでユーザーが Claude Code に「artgraph をこのプロジェクトに入れて」と頼むと、エージェントが 1 ターンで package manager 検出 → install (`npm` / `pnpm` / `bun` / `deno` のいずれか適切なもの) → `artgraph init` (デフォルトで full agent-native setup) → SDD ツール統合・Skills/Hooks/agent-context 配置までを完了する。ユーザーの追加操作は基本的に同意 (Y/N) のみ。
...
3. **Given** `.specify/` が存在するプロジェクト、 **When** ユーザーが「artgraph 入れて」と依頼する、 **Then** エージェントは Spec Kit の検出を報告し、統合 (Spec Kit extension 配備) も同セッション内で完了する (`init` デフォルトの `--integrations=all` が動く)。
...
**Independent Test**: 別 repo で `/plugin marketplace add ShintaroMorimoto/artgraph` と `/plugin install artgraph@artgraph-marketplace` を実行し、Skills が `~/.claude/plugins/cache/.../skills/` に配置され `/artgraph:check` 等の namespace 経由 invoke が動くことを確認。`templates/skills/artgraph-impact/SKILL.md` を 1 行変更し、両配布物に反映されることを確認。
...
3. **Given** artgraph repo で `templates/skills/artgraph-impact/SKILL.md` を 1 行編集して push、 **When** 新規ユーザーが (1)(2) を実行、 **Then** 編集が plugin 配布物にも反映されている (`templates/skills/` 直接参照のため)。
...
- **両 SDD ツール並存**: `.specify/` と `.kiro/` 両方が存在する repo で `init` (デフォルト `--integrations=all` 相当) は両方の統合を順次配備する (どちらかを優先せず両立)。
- **SDD ツールゼロ**: SDD ツール検出ゼロの repo で `--integrations=all` は no-op (エラーにせずスキップ)。
...
- **`artgraph-setup` で `npm`/`bun`/`deno`/`pnpm` 混在**: lockfile (`bun.lockb` / `deno.lock` / `pnpm-lock.yaml` / `package-lock.json`) または `package.json#packageManager` を見て、適切な install/exec コマンドを構築する。検出ゼロは npm 既定。Yarn は本 spec のサポート対象外 (検出されても npm fallback)。
...
- **FR-026**: `artgraph-setup` Skill MUST detect the project's package manager from lockfile presence (`bun.lockb` → Bun, `deno.json`/`deno.lock` → Deno, `pnpm-lock.yaml` → pnpm, `package-lock.json` → npm) or `package.json#packageManager` field, and construct install/exec commands accordingly (e.g., `bun install -D artgraph && bunx artgraph init` for Bun, `pnpm add -D artgraph && pnpm exec artgraph init` for pnpm). Detection-miss MUST default to npm. **Yarn is intentionally excluded** from supported targets in this spec (if `yarn.lock` is detected, the Skill falls back to npm and surfaces a note). (Wider package-manager-agnostic rewrites of Stop hook templates, Skill body samples, README, and Plugin hook are scoped to a follow-up issue.)
...
- **Package Manager Detector**: a utility (used inside `artgraph-setup` Skill) that inspects lockfiles and `package.json#packageManager` to determine which package manager (npm / pnpm / Bun / Deno) to use for install and exec commands. Yarn is excluded from this spec; if detected the Skill falls back to npm.
...
- **SC-001**: New-user "install artgraph" → first successful `artgraph check --gate` passes within **3 user inputs** (1 install consent + at most 2 follow-ups). Measured by manual walkthrough in a clean Vitest project across all 4 supported package managers (npm / pnpm / Bun / Deno).
...
- **SC-010**: `artgraph-setup` Skill successfully completes (install equivalent + `init` + Skills available) on test fixtures for **all 4 supported package managers** (npm / pnpm / Bun / Deno). Measured by 4 E2E fixture tests, one per package manager.
...
- **Package manager**: artgraph CLI itself is published as an npm package, but install/exec commands generated by `artgraph-setup` Skill adapt to npm / pnpm / Bun / Deno based on detected lockfile or `package.json#packageManager`. Fallback is npm. **Yarn is intentionally excluded** from this spec's support matrix (if a `yarn.lock` is detected the Skill falls back to npm and warns the user). Wider rewrites (Stop hook templates, README examples, Plugin hook) to be fully package-manager-agnostic are tracked in a follow-up issue.
...
- **`--integrations=all` semantics**: invokes all *detected* integrations sequentially with `--gate` enabled by default; users who want a different gate posture can re-run with explicit `--no-integrate-gate` or per-tool `integrate <tool>` later. `all` is the sentinel value used when `init` is invoked with no flags (renamed from the prior `--integrate=auto`).
