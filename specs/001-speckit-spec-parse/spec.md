...
- FR-007: 認識する ID パターンは設定（`.spectrace.json`）で拡張・制限できる (deferred: Phase 6)
- FR-008: Spec Kit frontmatter（title, status, priority, owner）を読み取り、仕様ノードのメタデータとして保持する (deferred: 将来バージョン)
