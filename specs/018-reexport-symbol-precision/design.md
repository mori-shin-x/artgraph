...
入力:
  ...
  starMap: Map<fileRel, targetRel[]>   // fragment の starExports を集約。
    // 集約時に解決済み targetRel で dedup する (初出順を保持)。同一モジュール
    // への重複 star 宣言 (`export * from "./o"` × 2 や、別スペシファイアが
    // 同一ファイルに解決するケース) は ES 仕様上「同一束縛」であり曖昧では
    // ない — dedup しないと |providers| ≥ 2 と誤判定し偽 ambiguous になる。
...
**メモ化 (必須)**: 素の再帰は不可。共通の下流 barrel を複数の親が star する
diamond を k 段重ねた DAG で呼び出しが段ごとに倍加し O(2^k) に爆発する
(monorepo の index.ts 再輸出チェーンで現実に起こる形)。一方、(F, name) 単位の
無条件メモ化は循環カット (手順 3) 越しに path 依存の結果を汚染しうるので不健全。
採用する規則は**条件付きメモ化**:

- 再帰中に手順 3 の循環カットに一度も触れずに完走した結果のみ (F, name) で
  キャッシュする (カットに触れたら結果を返すがキャッシュしない)。
- DAG (循環なし) ではすべてキャッシュされ、計算量は
  O(ファイル数 × 名前数 × star out-degree) の多項式に落ちる。真の循環に
  触れた query だけ再計算になるが、循環成分のサイズで有界。
- `exportedNames` も同じ規則でメモ化する (visited カットに触れなかった
  結果のみキャッシュ)。
...
**実体化の前提ガード (既存 import/reexport 処理と同一)**: S3 の実体化は
binding の specifier が `"."` 始まり (相対) で、かつ `resolveRelativeImport`
が解決に成功した場合のみ行う。bare specifier (`import X from "react";
export default X;`) や解決失敗は**実体化しない** (今日の挙動のまま skip)。
§4 の hash 入力が常に「解決済み rootDir-relative path」であることは
このガードが保証する。
...
- **S2**: `ExportAllDeclaration` で `exported` 非 null (`export * as ns`) の
  ...
  名前の取り出しは既存 `moduleExportName` を流用する —
  `export * as "文字列名" from` (ModuleExportName は string literal も許容、
  oxc 実測済み) は literal の値が名前になる。`exportKind` (type/value) は
  `export type { } from` の既存前例と同じく区別しない
  (`export type * from` / `export type * as ns from` も同扱い)。
...
3. **drop / none の帰結** — consumer の `import { x }` edge は phantom のまま
   ...
   drop された名前の下流挙動は**すべて「今日の star barrel の挙動」と同一**
   であることを明記する: `impact` / `check` は repair 経由の file 粒度で
   REQ 到達、`path:symbol` 入力 (`resolveStartIds`) は今日と同じ
   unresolvedSymbol エラー、`entryOriginIds` は symbol chain が繋がらず
   origin 帰属なし (impactReqs ⊃ originReqs の非対称は plan-coverage の
   implicit-impact 検知として今日も出る形であり、regression ではない)。

**仕様からの意図的逸脱 (D4 系)**: ES 仕様では「複数 star 経由でも究極の束縛が
同一なら曖昧でない」(diamond: A→{B,C}→D, D.x)。本設計は **dedup 後に異なる**
直近供給元が 2 つあった時点で drop する (束縛同一性の比較を実装しない。
同一 targetRel への重複 star は §5 の starMap dedup で単一供給元に畳まれる
ため曖昧にならない)。理由: (a) 供給元
...
| `path:symbol` 入力 (S2 / S3-namespace) | pre-018 は `symbol:B#ns` が graph 未登録で `resolveStartIds` が `unresolvedSymbol` として弾き CLI が「No matching symbol」で exit 1。post-018 は `symbol:B#ns` が materialize されるが `entryOriginIds` (traverse.ts) の `edge.target.startsWith("symbol:")` フィルタで `symbol:B#ns → file:O` edge が skip され `originReqs = []` → `impactReqs \ originReqs = impactReqs` が全件 drift 候補化。**ユーザー可視には「hard error → silent false-positive drift」への退化**。symbol → symbol での drift 計算では file 粒度エッジをフォローしない (現状維持) が、pre-018 の hard error のほうが accurate。将来 `entryOriginIds` を file → same-file symbols expansion に拡張するか、`path:symbol` 入力側で S2/S3-namespace を明示エラー化するかは別 issue |
...
- fatal syntax error ファイル: oxc-parser の
  `parsed.module.staticExports[].entries[].importName.kind` は `Name` /
  `All` / `AllButDefault` を区別可能で、原理的には `isPlainStar` /
  `nsName` を復元できる。**しかし** `parsers/typescript.ts:998-1008` の
  fallback はこの情報を捨てているため、実装として star / ns 区別を
  復元していない (原理的に不可能ではなく、fatal-syntax file は file 粒度
  維持で問題ないという設計判断による意図的な単純化) → 従来どおり file
  粒度 (#180 原則)。将来 fatal-syntax file の per-symbol 精度を上げるなら
  (別 issue)、`importName.kind` の抽出を追加すればよい。
- 曖昧 star (§7) と diamond-同一束縛 (D4): file 粒度 fail-safe。
- star ターゲットがスキャン範囲外 (exclude glob 等): 展開不可。consumer の
  named import は `file:B` (barrel 自体はスキャン内) への
  `phantom-import-repaired` 降格で閉じる。`dangling-import` が出るのは
  barrel の file node 自体がスキャン外の場合のみ (builder repair の既存分岐
  そのまま)。barrel の `file:B → file:O` edge は従来どおり silent に
  dead-end する。
...
| T7 | 曖昧: B: `export * from o1` + `export * from o2` (両方 x)、**C: `import { x } from B` を含める** (consumer 不在だと repair が動かず warning も出ない) | `symbol:B#x` 非実体化 → C の edge が repair され `phantom-import-repaired` 発火。`B:x` の `path:symbol` 入力は今日と同じ unresolvedSymbol エラー |
...
| T14 | `export type * from` / `export type * as ns from` / `export * as "文字列名" from` | 値 star と同様に展開・実体化 (`export type { } from` の既存前例と整合)。string literal は値が名前になる |
...
| T17 | S1 リファクタ等価性: B: `export * from O` の lock bytes ≡ B を明示列挙 `export { x } from O` (O の全 export 分) に書き換えた lock bytes | **lock byte 等価** (graph は file-grain edge の有無で差があるが lock は imports edge を含まないため一致 — §4 の pin) |
| T18 | 同一ターゲット重複 star: B: `export * from "./o"` ×2 (または別スペシファイアで同一ファイルに解決) | 曖昧扱いに**ならず**通常展開 (starMap dedup の検証) |
| T19 | S3 bare specifier: `import X from "react"; export default X;` | 実体化しない (今日の挙動のまま)。graph に `symbol:` node 追加なし |
| T20 | diamond 積層 DAG (共有下流 barrel を持つ 2 分岐 ×数段) | 正しい展開結果 + 現実的時間で完走 (条件付きメモ化の検証。fixture は決定的に生成) |
