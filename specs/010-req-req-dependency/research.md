...
- 衝突 ID で remap 候補が一意に決まらない場合（同名 ID が複数 specDir に存在）、
  既存挙動と同様に警告 `ambiguous-target` を出し edge は生成しない
...
| `ambiguous-annotation-target` | 注釈で参照された ID が複数 specDir で衝突し remap 不能 | builder（既存 `ambiguous-target` の派生で扱うか別 type かは plan 細目で決定。デフォルトは既存型を流用） |
