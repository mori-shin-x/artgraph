...
- Q: `task` NodeKind 追加で NodeKind が増加するが、将来の進化に対するガードレールは?（CV1） → A: **本 feature 内での限定的逸脱**として扱う。前提として **Constitution Principle II は 4 抽象層（req / doc / code / test）を宣言**しており、現行実装はそのうち code 層を `file` と `symbol` の 2 NodeKind で表現しているため **現行 5 NodeKind**（`req` / `doc` / `file` / `symbol` / `test`）。本 PR ではここに `task` を追加し **6 NodeKind** とする。Constitution 本文（4 抽象層宣言）は本 PR で変更しない（4 抽象層 / 5 NodeKind の現状記述と、6 NodeKind 化の限定逸脱を並存させる）。今後さらに NodeKind を追加する PR は同様に Complexity Tracking で個別 justify を要求し、累積追跡は本 PR の PR 本文 + フォローアップ tracking issue (作成タスク T030) で行う。累積 3 件目の NodeKind 追加時に Constitution 改訂を検討する。
...
- **FR-009** _(In Progress)_: `@impl(target-id)` タグを認識し、`task` ノードから `target-id` への `implements` エッジを生成する。`task` は新規 NodeKind（`types.ts` の `NodeKind` union に追加）。`target-id` は自由形式文字列として扱い、parser 段では存在確認を行わない（builder 段で突き合わせ、未解決時は `orphan-doc` 系警告）。**認識対象ファイル**: [Clarifications U1](#clarifications) を参照（`taskConventions` プリセット適用ファイル全て）。詳細 Acceptance: [US3 §1, §5](#user-story-3---planmd--tasksmd-とコードの紐付き-priority-p3----in-progress-issue-28)
- **FR-010** _(In Progress)_: `[REQ-xxxx]` タグを認識し、`task` ノードからテスト対象要件への `verifies` エッジを生成する。target ID は **bracket 内文字列をそのまま** 使用（`REQ-` prefix は剥がさない、例: `[REQ-FR-001]` → target = `REQ-FR-001`）。これにより TS パーサ (`typescript.ts:254`) と同 ID 形式となり、`coverage.ts` / `lock.ts` の既存ロジックを再利用できる。**認識対象ファイル**: [Clarifications U1](#clarifications) を参照。詳細 Acceptance: [US3 §2, §4](#user-story-3---planmd--tasksmd-とコードの紐付き-priority-p3----in-progress-issue-28)
- **FR-012** _(In Progress)_: タスク ID 抽出は **規約プリセット方式** で行う。Built-in プリセット: `spec-kit`（ファイル stem `plan` / `tasks` / ID 正規表現 `T\d+` 系）、`kiro`（同 stem `tasks` / 階層数字 `\d+(?:\.\d+)*`）。`.artgraph.json` の `taskConventions: TaskConventionPreset[]` フィールドで OpenSpec 等のカスタムプリセットを追加可能。検出はファイル名 stem 一致で行い、複数プリセットが該当した場合は両方を適用、エッジ dedup（`source|target|kind`）で重複排除する（C-3 `inferConventionEdges` と同じ思想）。
