...
- **FR-015** (issue #356 で追加、PR #359 レビューで判定方式を更新): `artgraph doctor` は `.artgraph.json` の `include` / `testPatterns` それぞれの node_modules 除外用負パターンの実効性を診断し、advisory (severity `pass`、doctor の終了コードを変更しない) として報告する。判定は代表合成相対パス (`node_modules/x.ts` / `a/node_modules/x.ts` / `a/b/node_modules/x.ts`) を picomatch (fast-glob の ignore 評価と同一オプション設定) で各 pool の負パターンに照合する構造的診断であり (ファイルシステムアクセスなし・完全に決定的、Constitution Principle V に整合)、単純な文字列・パスセグメント比較ではなく実際の glob マッチング結果に基づく。報告対象は次の 2 カテゴリ: (a) **非対称** — 両 pool が判定対象 (後述) かつ一方のみが全ての代表パスで保護されている場合、保護されていない側を報告する、(b) **壊れた除外** — pool の負パターンが node_modules という語句を含む (文字列・パスセグメント一致) にもかかわらず、matcher 判定では全代表パスを保護できていない場合 (例: `!node_modules/**` はネストしたパスを保護しない) — 対称性の有無に関わらず報告する。ポジティブパターンを 1 つも持たない pool (空配列 / 全要素が負パターン。issue #266 によりそのような pool は何もマッチしない) は両カテゴリの判定対象から除外する。両 pool とも node_modules への言及が無く、かつ判定対象である場合は診断対象としない (vendor コードを意図的にスキャンする等の対称な選択と区別できないため)。既存の `config-missing-agents-field` (spec 013 follow-up #158) と同じゲート条件 (少なくとも 1 Tier 1 エージェントが検出されていること) の下で評価する — 配布が 1 件も無いプロジェクトでは doctor 自体が空レポートを返す既存の短絡経路を維持するため。
