...
| `copilot` | GitHub Copilot (IDE / CLI / Coding Agent 共通) | — (**配布対象外**、issue #130) | `AGENTS.md` | `.github/copilot-instructions.md` |
| `kiro` | Kiro | `.kiro/skills/` | `AGENTS.md` | — (ラッパーなし、AGENTS.md ネイティブ。`.kiro/steering/artgraph.md` は **別責務** = `--integrations=kiro` 担当) |

**Copilot の Skills 配布を skip する理由 (issue #130):**
GitHub Copilot が custom instructions として認識する公式パスは `.github/copilot-instructions.md` (repo-wide) と `.github/instructions/*.instructions.md` (path-scoped) の 2 系統のみ。`.github/skills/` は README・IDE・Coding Agent いずれの surface でも discovery 対象外で、そこに SKILL.md を置いても Copilot は実行時に参照しない。旧実装 (spec 013 T003) は `.github/skills/` に配布していたが、これは false-green (doctor は緑、Copilot は認識せず) を引き起こす。Copilot は AGENTS.md をネイティブロードするため、Skills 一覧と使い方は AGENTS.md 本文 + `.github/copilot-instructions.md` ラッパー経由で伝える方針とし、Skills 配布は明示的に skip する。
...
│                                          ※ Copilot は Skills 配布対象外 (issue #130)
└── .kiro/
    └── skills/                            ← kiro Skills
...
合計 11 ファイル/エージェント (= 8 Skill SKILL.md + 3 `_shared/` 部品) × 4 エージェント (Claude / Codex / Cursor / Kiro) = 最大 44 ファイル配布。Copilot は Skills 配布対象外のため `.github/skills/` 系統は作成されない (issue #130)。
...
| パス | 理由 |
|---|---|
| `.github/skills/` (Copilot) | issue #130 — Copilot 公式 discovery 対象外。旧版が作成した残存 dir は `artgraph doctor` が `legacy-copilot-skills-path` で通知 (手動削除推奨) |
