...
| `skillsPath` | `string` (repo-root relative) | canonical Skills 配布先 (`.claude/skills/` ほか) |
...
| `copilot` | GitHub Copilot | `.github/skills/` | `.github/copilot-instructions.md` | `both` |
| `kiro` | Kiro | `.kiro/skills/` | `null` | `native-agents-md` |

**不変条件**:
- `id` は同一 table 内で一意。
- `skillsPath` は `<agent_key>/skills/` 形式に正規化される (POSIX セパレータ)。
...
| `dstAbsPath` | `string` | 配布先絶対パス (`<repo>/<agent.skillsPath>/<srcRelPath>`) |
| `expectedSha256` | `string` | `SkillFile.sha256` (配布後の期待値) |

**生成ルール**:
- `--agents=<list>` で指定された各 `AgentDescriptor` × `SkillSource.entries[].files[]` の直積で `DistributionTarget[]` を作る。
