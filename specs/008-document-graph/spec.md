...
1. Given design.md の frontmatter に `derives_from: requirements.md` と記述されている, When `spectrace scan` を実行する, Then design.md から requirements.md への `derives_from` エッジがグラフに生成される
...
3. Given `spectrace graph --kind doc` を実行する, When doc ノードのみの依存関係を見たい場合, Then doc ノードに限定したグラフが表示される
...
- FR-001: 各 Markdown ファイルを frontmatter の有無に関わらず `doc` ノードとしてグラフに自動登録する。frontmatter `spectrace.node_id` がある場合はその値を ID として使用し、無い場合は spec ディレクトリからの相対パスを基に自動採番する
- FR-002: ドキュメント間の依存関係を frontmatter の `spectrace.depends_on` で表現する。relation として `derives_from` と `depends_on` の 2 種類をサポートし、それ以外の値には `invalid-relation` 警告を出す
...
- FR-005: frontmatter の依存先ノードが存在しない場合、`orphan-doc` 警告を出力する
...
- FR-008: doc ノードの自動生成と contains エッジ生成はそれぞれ独立した設定で有効・無効を制御できる
- FR-009: `impact` 出力に到達ノード数の内訳（docs / reqs / files）を表示し、contains 経由の影響範囲の広がりを可視化する
...
- derives_from エッジ: 下流ドキュメントから上流ドキュメントへの派生関係（例: design.md が requirements.md から派生）
- depends_on エッジ: ドキュメント間の一般的な依存関係
...
- `via` エッジメタデータは v1 では追加しない（消費者がいないため）
...
- 未リリースのため後方互換は考慮しない
- impact の到達深さ制御は v1 では入れない（必要なら後続で対応）
