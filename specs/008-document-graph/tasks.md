...
- [ ] T003 `src/types.ts` の `ArtgraphConfig` に `docGraph?: DocGraphConfig` フィールドを追加する
...
- [ ] T008 [P] `tests/fixtures/specs/doc-chain/requirements.md` を作成する: frontmatter に `artgraph: { node_id: "requirements" }` を記述
- [ ] T009 [P] `tests/fixtures/specs/doc-chain/design.md` を作成する: frontmatter に `artgraph: { node_id: "design", derives_from: ["requirements"] }` を記述
- [ ] T010 [P] `tests/fixtures/specs/doc-chain/tasks.md` を作成する: frontmatter に `artgraph: { derives_from: ["design"] }` を記述
- [ ] T011 [P] `tests/fixtures/specs/doc-with-reqs.md` を作成する: frontmatter に `artgraph: { node_id: "auth-spec" }` を記述し、本文に `- FR-001: ...` リスト項目を含む
...
- [ ] T012 `tests/config.test.ts` に `docGraph` 設定テストを追加する: `.artgraph.json` に `{ "docGraph": { "autoNodes": false } }` を指定した場合に `config.docGraph.autoNodes === false` が返ること
...
- [ ] T014 `tests/markdown.test.ts` に frontmatter フラット化テストを追加する: `artgraph: { derives_from: ["requirements.md"] }` が `derives_from` エッジを生成すること（旧 `depends_on: [{ id, relation }]` 形式からの移行）

### Implementation for Foundational

- [ ] T015 `src/config.ts` の `loadConfig` に `docGraph` の読み込みを追加する: `raw.docGraph` を `ArtgraphConfig.docGraph` にマッピング
- [ ] T016 `src/parsers/markdown.ts` の `artgraphMeta` 型定義をフラット形式に変更する: `{ node_id?: string; derives_from?: string[]; depends_on?: string[]; [key: string]: unknown }` 。旧形式 `depends_on: [{ id, relation }]` のパースロジックを新形式に置き換える
...
- [ ] T018 `src/parsers/markdown.ts` に `invalid-relation` 警告を追加する: `artgraph` ブロック内の `node_id` / `derives_from` / `depends_on` 以外のキーを検出し `ParseWarning` として返す
...
Independent Test: frontmatter を含まない散文のみの md ファイルを配置し `artgraph scan` で doc ノードとして認識されることを確認する
...
Goal: frontmatter の `artgraph` ブロックで doc→doc 依存（derives_from / depends_on）を宣言し、グラフに反映する（FR-002, FR-005, FR-006）

Independent Test: frontmatter で derives_from 関係を記述した 3 ファイルの連鎖を配置し、`artgraph scan` で doc→doc エッジが正しく張られることを確認する
...
Goal: `artgraph graph` コマンドでドキュメント依存チェーンを text / JSON 形式で出力する（FR-004）

Independent Test: doc 依存チェーンを持つプロジェクトで `artgraph graph` を実行し、text / JSON 両形式で依存構造が正しく出力されることを確認する
...
- [ ] T054 [P] [US4] `tests/cli.test.ts` に `artgraph graph` コマンドの統合テストを追加する: CLI 経由で graph コマンドが実行でき、text/JSON 形式で出力されること
...
Checkpoint: `pnpm test` が通り、`artgraph graph` コマンドが text / JSON 形式で依存構造を出力する
