...
spec 022 実装レビューの事前警告どおり、`node:inspector` の `Profiler.startPreciseCoverage` → 対象コードを実行 → `takePreciseCoverage` で `functionName` を直接観測する probe(getter/setter・static method・generator・async・computed key・配列内無名関数・`export default` 無名/有名を実行)を作成し、Node v22.22.2 で実測した。結果は 3 カテゴリに分かれる。
