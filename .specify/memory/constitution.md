...
Ratified: 2026-06-23
Initial ratification — no prior principles to compare against.
...
- .specify/templates/tasks-template.md      ✅ サンプルタスクは generic、ユーザストーリー駆動の構造は原則 III と整合
- .specify/templates/commands/*.md          ✅ 本リポジトリでは .claude/skills/speckit-*/ に配置 — 該当パス無し
- README.md / docs/spectrace-design.md      ✅ 原則は両者から導出。逆方向の更新は不要
...
- **Monorepo**: pnpm workspace。`packages/artgraph` が CLI 本体、追加パッケージは
  ルート `pnpm-workspace.yaml` で管理する。
- **AST 解析**: ts-morph を一次解析として採用する。`import *` / 動的 import /
  リフレクション等で symbol-level に解決できないエッジは file-level にフォール
  バックし、グラフモードを出力に明示する。
- **Markdown 解析**: unified + remark-parse、frontmatter は gray-matter。
