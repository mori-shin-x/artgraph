...
Identify your own skills path among the five canonical locations: `.claude/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, `.kiro/skills/`. Check every one of these paths that exists in the project — multi-agent projects may have several — and in each, list the subdirectories whose names start with `artgraph-`. If none of the five paths exists, report that no Skills are distributed in this project.
