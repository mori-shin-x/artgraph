...
- Whether the Stop hook is wired: `.claude/settings.json` exists and its `hooks.Stop` array contains an entry whose `command` includes `check --gate --diff` (the artgraph gate rendered from `templates/hooks/settings.json.template`) -> `hook: wired` or `hook: missing`.
- Whether the agent-context snippet is present for each target agent (see Step 2.5 for how the target agents are decided): `AGENTS.md` exists at the project root with an intact `<!-- artgraph:begin -->` / `<!-- artgraph:end -->` marker block, and — for agents whose descriptor carries a wrapper file (`claude` -> `CLAUDE.md`, `copilot` -> `.github/copilot-instructions.md`) — that wrapper also exists with its own intact marker block importing `@AGENTS.md` -> `agent-context: wired` or `agent-context: missing`.
...
If the Stop hook or the agent-context snippet is missing, run the **init** command from the Step 3 table with `--force --agents=<list> --no-scan --no-skills --no-integrate` appended instead (`<list>` is the agents decided per Step 2.5). This is safe to run: `--force` only changes how `.artgraph.json` is regenerated — it merges the user's existing customizations rather than overwriting them — and the hooks stage never overwrites an already-populated `hooks.Stop`, with or without `--force` (settings-merge contract Case D always refuses to touch a pre-existing Stop hook). So running this command when only one of the two is missing will not disturb the other or clobber anything already wired.
