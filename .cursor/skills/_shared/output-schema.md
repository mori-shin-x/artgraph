...
`warnings[].type` is one of: `"duplicate-id" | "ambiguous-id" | "orphan-doc" | "orphan-edge" | "invalid-relation" | "reserved-prefix" | "unresolved-link" | "out-of-scope-link" | "invalid-annotation-id" | "empty-annotation" | "self-reference-annotation" | "phantom-import-repaired" | "dangling-import" | "class-member-collision" | "pathological-bracket-nesting" | "unreadable-file" | "system-resource-exhausted" | "node-modules-in-scan" | "unresolved-reexport" | "unresolved-import" | "config-pool-protection-asymmetry"`. This list may grow in future releases — a consumer should ignore any `type` value it does not recognize rather than treat it as an error.

`config-pool-protection-asymmetry` is silent (JSON-only, like `unresolved-reexport` / `unresolved-import` above): `include` has a `"!**/node_modules/**"`-style negative pattern but `testPatterns` doesn't, or vice versa. Fires on every scan of an asymmetric config regardless of whether the project has a node_modules directory at all — a purely structural config-shape check, unrelated to `node-modules-in-scan` above (which requires an actual matched file). A config where BOTH pools lack the negation never fires this (indistinguishable from a deliberate symmetric choice, e.g. intentionally scanning vendored code).
